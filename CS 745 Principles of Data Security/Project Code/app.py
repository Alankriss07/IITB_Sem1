...
# using sendinblue to provide email functionality
def send_email(ver,rec):
    ...
    data = {
        "sender": {"name": "Somil", "email": "alannkriss07@gmail.com"},
        "to": [{"email": rec}],
...
def login():
    error = None
    # if a new response is opened than clear whatever is there on the screen
    if request.method=='GET': 
        session.clear()
    if request.method == 'POST':
        # taking email and password from the form
...
#generating verification code for two factor authenrication
...
#session started
            ...
            # ABSENCE OF THIS LINE CAN CAUSE EXPLOITATION as session is not logged out so two factor authentication is kind of useless as a hacker can bypass it as the session is logged in
